...
class Tracker:
    ...
    def update(self, detections, classes, confidences):
        ...
        for detection_idx in unmatched_detections:
            ...
            matched_id, distance = self.metric.find_matching_deleted_id(feature)
    ...
    def _reactivate_track(self, track_id, detection, class_id, conf):
        """Reactivate a previously deleted track with its original ID.
        
        This enables persistent re-identification: when a person returns
        after leaving the frame, they get their original ID back.
        
        Parameters
        ----------
        track_id : int
            The original track ID to reactivate.
        detection : Detection
            The detection to initialize the reactivated track.
        class_id : int
            The class ID of the detection.
        conf : float
            The confidence score of the detection.
        """
        # Reactivate the ID in the metric
        self.metric.reactivate_id(track_id)
        
        # Create new track with the old ID
        self.tracks.append(
            Track(
                detection.to_xyah(),
                track_id,  # Use the old ID instead of _next_id
                class_id,
                conf,
                self.n_init,
                self.max_age,
